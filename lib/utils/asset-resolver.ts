...
/**
 * Gets the hero icon URL from Valve CDN (with alias applied)
 * 
 * @param heroId - The hero ID from Stratz data
 * @returns The CDN URL for the hero icon, or null if hero not found
 */
export function getHeroIconUrl(heroId: number): string | null {
  ...
  return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/heroes/${assetKey}_icon.png`;
}
 ...
 * 
 * @param heroId - The hero ID from Stratz data
 * @returns The fallback CDN URL, or null if hero not found
 */
export function getHeroIconFallbackUrl(heroId: number): string | null {
  ...
  if (originalKey !== aliasKey) {
    return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/heroes/${originalKey}_icon.png`;
  }
  return null;
}

/**
 * Gets the hero full portrait URL from Valve CDN
 * 
 * @param heroId - The hero ID from Stratz data
...
export function getHeroFullUrl(heroId: number): string | null {
  ...
  const assetKey = getAssetKey('hero', normalizeToAssetKey(hero.displayName));
  return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/heroes/${assetKey}_full.png`;
}

/**
 * Gets the ability icon URL from Valve CDN (with alias applied)
 * 
...
export function getAbilityIconUrl(abilityId: number): string | null {
  ...
  return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/spellicons/${assetKey}.png`;
}
...
export function getAbilityIconFallbackUrl(abilityId: number): string | null {
  ...
  // Only return fallback if alias was applied (keys differ)
  if (originalKey !== aliasKey) {
    return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/spellicons/${originalKey}.png`;
  }
  return null;
}

/**
 * Gets the item icon URL from Valve CDN (with alias applied)
 * 
...
export function getItemIconUrl(itemId: number): string | null {
  ...
  return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/items/${assetKey}_lg.png`;
}
...
export function getItemIconFallbackUrl(itemId: number): string | null {
  ...
  // Only return fallback if alias was applied (keys differ)
  if (originalKey !== aliasKey) {
    return `https://cdn.cloudflare.steamstatic.com/apps/dota2/images/items/${originalKey}_lg.png`;
  }
  return null;
}
